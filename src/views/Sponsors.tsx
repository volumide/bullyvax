			...
			if (res) {
				res.forEach((result: any) => {
					if (result?.message || result?.sponsors) {
						const individialCheck = result && result?.message ? result.message.split(" ").pop().toUpperCase() : "";
						if (individialCheck && individialCheck.toLowerCase() !== "individual") exist.push(result);
					}
				});
			}
		} catch (error) {
		...
		if (exist.length > 0) {
			...
			exist.forEach((all) => {
				if (all.sponsors.length >= 4) {
					sponsorOverlow.push(`${all?.sponsors[0].school_name.toUpperCase()} reached maximum number of sponsors ${all?.sponsors.length}`);
				}
			});
	...
	let handleSeachK = async () => {
		...
		try {
			...
			if (res.data?.length === 0) {
				setModalContent(
					<div>
						Your school is currently not protected by BullyVaxx. All that is needed for your school to become protected is a individual or business to
						step up and become the sponsor for the school. Real estate agents, new and used auto dealerships, personal injury attorneys, restaurants
						and church youth groups all make great sponsors for BullyVaxx. Please contact any of these businesses/groups that you are connected to and
						get your school protected. To sponsor a school please click <Link to="/sponsors">HERE</Link>.
					</div>
				);
			} else {
				setModalContent(
					<div>
						Yes, your school is protected by BullyVaxx,sponsored by businesses in ({businesses}) to file a bully or threat report please click{" "}
			...
			<Grid container spacing={2}>
				...
				<Grid item sm={4} xs={12}>
					<Box sx={{ marginTop: "4%", padding: "2%" }}>
									...
									<>
										...
										<FormFieldWrapper>
											<FormControl fullWidth>
												...
												<Select labelId="demo-simple-select-label" id="demo-simple-select" value={description} label="Age" onChange={handleChange}>
													{businessOptions &&
						...
						{existing.length > 0 ? (
							<div className="border rounded p-2" style={{ border: "1px solid grey", borderRadius: "2px", padding: "5px" }}>
						...
						{sponsorsOverflow.length > 0 && (
							<div className="border rounded p-2" style={{ border: "1px solid grey", borderRadius: "2px", padding: "5px", marginTop: "10px" }}>
						...
						<div>
							<p>
								BullyVaxx sponsors elevate their business reputation to the very highest level by making it possible for thier community's schools to
								transition into bully-free schools
							</p>
						</div>
					</Box>
				</Grid>
